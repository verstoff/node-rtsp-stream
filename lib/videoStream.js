...
(function() {
  ...
  VideoStream.prototype.startMpeg1Stream = function(callback) {
    var started = false;
    var gettingInputData, gettingOutputData, inputData, outputData, self;
    this.mpeg1Muxer = new Mpeg1Muxer({
      url: this.streamUrl,
      rtspTransport: this.rtspTransport || 'tcp',
      bitrate: this.bitrate || '800k',
    ...
    this.mpeg1Muxer.on('ffmpegError', function(data) {
      ...
      if (gettingInputData) {
        ...
        if (size != null) {
          ...
          if (self.height == null) {
            self.height = parseInt(size[1], 10);
          }
          !started && (started = true) && callback && callback();
        }
      }
    });
    return this;
  ...
  VideoStream.prototype.onSocketConnect = function(socket) {
    ...
    self = this;
    if (!self.mpeg1Muxer) {
      self.startMpeg1Stream(function() {
        self.sendHeader(socket);
      });
    } else {
      self.sendHeader(socket);
    }
    ...
    return socket.on("close", function(code, message) {
      if (!self.always && !self.wsServer.clients.length) {
        self.mpeg1Muxer.stream.kill();
        delete self.mpeg1Muxer;
      }
      return console.log(("" + self.name + ": Disconnected WebSocket (") + self.wsServer.clients.length + " total)");
    });
  };

  VideoStream.prototype.sendHeader = function(socket) {
